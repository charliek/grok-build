...
pub(crate) async fn gx_flush_leader_sessions_on_signal(is_leader: bool) {
    if !is_leader {
        return;
    }
...
async fn run_agent_command(
    ...
    // gx: only a leader has session actors whose `SessionEnd` hooks must run before the
    // process exits. Passed into upstream's `spawn_signal_flush` so the flush still
    // happens before `shutdown_and_flush_telemetry` (see `gx_flush_leader_sessions_on_signal`).
    let is_leader = matches!(agent_args.mode, Some(AgentCmd::Leader(_)));
    let signal_flush = agent_command::spawn_signal_flush(is_leader);
    if matches!(
        agent_args.mode,
