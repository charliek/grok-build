...
impl AuthProviderRef {
    ...
    pub fn new(name: String, config: AuthProviderConfig) -> Self {
    ...
    pub fn fail_closed(name: String) -> Self {
        Self {
            name,
            config: AuthProviderConfig::default(),
            slot: ProviderSlot::default(),
...
async fn mint_provider_token_with(
    ...
    let cwd = config
        ...
        .map(str::trim)
        .filter(|c| !c.is_empty())
        .map(xai_grok_shell_base::util::expand_home);
    ...
    let mut cmd = match config.args {
        ...
        None => xai_grok_shell_base::util::subprocess::shell_c(config.command.as_str()),
    };
    ...
    let parsed = match parse_token_output(&output) {
        ...
        Err(e) => {
            ...
            anyhow::bail!(
                "{e} (stderr: {})",
                xai_grok_shell_base::util::truncate(stderr.trim(), 300)
            );
        }
    };
    let expires_at = parsed
        ...
        .or_else(|| crate::parse_jwt_expiration(&parsed.access_token));
    tracing::info!(
        provider = %name,
        mark_expired,
...
fn map_in_process_minted_token(
    ...
    let expires_at = u64::try_from(minted.expires_in)
        ...
        .or_else(|| crate::parse_jwt_expiration(&token));
    tracing::info!(
        provider = %name,
        expires_at = ?expires_at,
        "auth provider minted token"
...
impl ProviderRefreshOutcome {
    pub fn rotated(self) -> Option<String> {
        match self {
...
#[cfg(any(test, feature = "test-support"))]
pub fn test_backdate_provider_mint(name: &str, age: std::time::Duration) {
    let slot = provider_slot(name);
    let mut slot = slot
        .try_lock()
