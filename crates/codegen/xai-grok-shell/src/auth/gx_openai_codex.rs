...
pub const LOCK_TIMEOUT: Duration = Duration::from_secs(90);

/// How often a waiter re-tries a contended `.gx-auth.lock`.
...
/// claims for scheduling; the server is the only thing that validates the token.
...
/// Pre-open gate: refuse anything that is not a bounded regular file (a fifo
/// would block forever; a device node would never end).
...
impl CodexPaths {
    ...
    /// gx home.
    pub fn for_auth_json(auth_json: PathBuf, grok_home: &Path) -> Self {
...
pub struct AuthDocument {
    ...
    pub(crate) openai_api_key: Option<serde_json::Value>,
...
pub fn read_auth_document(path: &Path) -> Result<AuthDocument> {
    if !gate_file(path, MAX_AUTH_JSON_BYTES, "auth.json")? {
        bail!(
...
// ---------------------------------------------------------------------------
// cross-process lock (gx-owned `.gx-auth.lock`, never providers.toml)
// ---------------------------------------------------------------------------

/// Held for the whole refresh; `Drop` releases the flock.
#[derive(Debug)]
...
/// Exclusive advisory flock on `.gx-auth.lock`. Path- and timeout-injectable
/// so tests can hold the same lock mint waits on.
pub(crate) fn lock_auth_at(lock_path: &Path, timeout: Duration) -> Result<AuthLock> {
    ...
    #[cfg(unix)]
    {
        ...
        // predates it too (best effort — it holds no token itself).
        let _ = std::fs::set_permissions(lock_path, std::fs::Permissions::from_mode(AUTH_MODE));
        ...
        loop {
            ...
            if std::time::Instant::now() >= deadline {
                bail!(
                    "another gx process is holding {} (waited {:?}); \
                     retry once it finishes, or remove the lockfile if no gx process is running",
                    lock_path.display(),
                    timeout
                );
            }
...
pub(crate) fn mint_token(opts: &MintOptions<'_>) -> Result<MintedToken> {
    ...
    let _lock = lock_auth_at(&paths.lock, opts.lock_timeout)?;
    let doc = read_oauth_document(&paths.auth_json)?;
    let now = opts.clock.now_unix();
...
///
/// Windows is a declared non-goal here: [`lock_auth_at`]'s
...
pub(crate) fn read_recovery_journal(path: &Path) -> Option<RecoveryJournal> {
    if !gate_file(path, MAX_AUTH_JSON_BYTES, "auth recovery journal")
        .ok()
        .unwrap_or(false)
    {
        return None;
...
pub fn read_state(path: &Path) -> GxCodexState {
...
pub fn account_check(current: Option<String>, cached: Option<String>) -> AccountCheck {
...
pub fn record_account(path: &Path, account_id: Option<&str>) {
