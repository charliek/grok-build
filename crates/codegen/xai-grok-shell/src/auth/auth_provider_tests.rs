...
struct CodexStore {
    ...
    paths: crate::auth::gx_openai_codex::CodexPaths,
}
...
impl CodexStore {
    fn with_body(body: &str) -> Self {
        ...
        let paths = crate::auth::gx_openai_codex::CodexPaths::for_auth_json(auth_json, grok.path());
        Self {
            _codex: codex,
            _grok: grok,
            paths,
    ...
    fn missing() -> Self {
        let codex = tempfile::tempdir().expect("codex home");
        let grok = tempfile::tempdir().expect("grok home");
        let paths = crate::auth::gx_openai_codex::CodexPaths::for_auth_json(
