...
9. **On any conflict you did not expect, ABORT — do not resolve it blind.**
   ...
   **Recurring conflict points.** These are the hunks that live inside upstream-owned
   ...
   | `crates/codegen/xai-grok-shell/src/leader/protocol.rs` | `ClientCapabilities.observer` and `hook_env` (`#[serde(default)]`) and `LeaderCapabilities::observer_v1`. Keep both when upstream adds a new capability field (e.g. `user_message_echo`, `cursor_worker`). |
   ...
   | `crates/codegen/xai-chat-state/src/commands.rs` | `SamplingConfig` / `SamplerConfig` / `ModelEntryConfig` **Default impls** (and any remaining exhaustive literals) need gx's trailing `codex_compat` / `hoist_tool_images` / `tool_result_images` / `supports_vision` fields when upstream adds a new field. Prefer taking upstream's `..Default::default()` in test literals. |
   ...
   | `crates/codegen/xai-grok-pager-bin/src/main.rs` | four hunks: `Command::Remote(_)` in **both** early-exit match lists, the `Command::Remote(…)` dispatch arm, and the `gx_remote_lane::spawn` line in `AgentCmd::Leader`. Keep `AgentRuntime::from_args` (upstream 1.0.35) and thread `is_leader` into `spawn_signal_flush` so SessionEnd still runs on SIGTERM. The two match lists are exhaustive over `Command`, so an upstream variant addition conflicts here by construction. |
   | `crates/codegen/xai-grok-pager-bin/src/agent_command.rs` | `spawn_signal_flush(is_leader)` plus the two `gx_flush_leader_sessions_on_signal` calls. Upstream 1.0.41 added `Drop` + `CancellationToken` + `SignalStreams`; keep those and still flush before `shutdown_and_flush_telemetry`. |
   | `crates/codegen/xai-grok-update/src/auto_update.rs` | `get_installer()` is a thin wrapper over `get_installer_for(is_gx)`. A gx build returns `None` before any installer detection. Upstream 1.0.41 added a WinGet path that outranks npm hints — keep that body **inside** `get_installer_for` after the `is_gx` early return. |
   | `crates/codegen/xai-grok-shell/src/agent/mod.rs` | `pub(crate) mod reasoning_family` next to upstream's `proxy_headers`. Keep both; they are independent modules. |
   | `crates/codegen/xai-grok-shell/src/agent/mvp_agent/session_setup.rs` | reconnect-rail `gx_hook_env` stamp beside `UpdateAttachPolicy`. Upstream 1.0.41 clones `initial_client_mcp_servers`; keep the `.clone()` **and** the gx block. |
   | `crates/codegen/xai-grok-shell/src/leader/lock.rs` | `leader_file_stem_for` / `gx-leader` vs `leader`. Upstream 1.0.41 routed acquire through `xai-grok-file-lock`; keep the stem helpers on the new lock body. |
