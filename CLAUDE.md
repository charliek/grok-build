...
- **OpenAI/Codex credentials:** mint/lock live in
  `crates/codegen/xai-grok-shell/src/auth/gx_openai_codex.rs` (in-process seam
  in `auth_provider.rs`). Pager `openai_codex_auth.rs` is a thin façade plus
  `run_login` (`codex` PATH lookup is pager diagnostics).
...
- **A provider key is never accepted as a command-line argument.** `gx providers
  ...
  same rule.
- Same rule for the OpenAI/codex path: `xai-grok-shell/src/auth/gx_openai_codex.rs`
